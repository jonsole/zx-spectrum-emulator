# Knight Lore on the Filmation engine

A VS Code workspace holding a remake of Ultimate's *Knight Lore* (1984), written
...
- **Your own copy of Knight Lore**: a snapshot of it -- .sna or .z80 -- taken at the menu, before a game is started.
...
Knight Lore** and press Enter at the prompt (or type the path to a copy
elsewhere).
...
nothing of it is kept. Knight Lore's tape is no use as it is: its loader decodes the game as it loads. Open the tape in VS Code and press Run; once the game's menu is up, run **ZX Spectrum: Save Snapshot...** and extract from the snapshot that makes.

## Running it

**Run > Start Debugging** (F5) builds the game and runs it. Click the **ZX
Spectrum Screen** panel and play from the keyboard:
`examples/filmation/knightlore/driving.md` has the keys. Pause (F6) to stop
...
`examples/filmation/knightlore/` opens its editor --
...
| | |
|---|---|
| `examples/filmation/engine/` | The Filmation engine, shared with the Pentagram remake |
| `examples/filmation/knightlore/` | The game's code, `build.py`, and `graphics.json` -- which sprite each graphic number draws and the nudge that places it, the remake's own |
| `examples/filmation/extract.py`, `original.py` | The extraction from your copy |
| `examples/filmation/knightlore/original.json` | The hashes a right copy gives -- hashes only, nothing of the game |
...
engine and the remake's code are MIT-licensed; *Knight Lore* itself is copyright
