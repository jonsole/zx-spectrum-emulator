...
async def profile(args):
    ...
    out_dir = build / "examples" / "filmation" / "knightlore128" / "output" \
        if (build / "examples").is_dir() else build / "output"
    z80 = (out_dir / "knightlore128.z80").read_bytes()
    sld = str(out_dir / "knightlore128.sld")
    asm = str(out_dir.parent / "knightlore128.s")
...
def main():
    ...
    p.add_argument("--build", default=str(HERE), help="a tree, or a knightlore128/ folder, holding output/knightlore128.z80")
