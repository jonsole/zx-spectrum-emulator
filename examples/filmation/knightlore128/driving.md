...
build, so they're always read from `output/knightlore128.sld`. The original's
...
Build with `python build.py` in this folder, which writes `output/knightlore128.z80`
and `output/knightlore128.sld`. `python build.py --debug-room` prints the room
...
```sh
grep -E "\|F\|room_number$" output/knightlore128.sld | cut -d'|' -f6
```
...
1. `pause`, `load_snapshot` with `output/knightlore128.z80` (base64 -- 45 KB, so
   from a script), `load_debug_info` with the SLD and `knightlore128.s`, and
   `set_speed uncapped`.
